...
<p>The likelihood of a risk materializing, and its associated business impact, depend on the type of data, its location, and whether the security team knows about it.</p>
<p>This <strong>Function</strong> can involve a variety of technical tools as well as more staff-driven processes such as meeting with internal employees and leaders to identify important data for each business unit.</p>
...
<li>
<p>How does the organization quantify the damage caused by a breach or data security event?</p>
</li>
<li>
...
<h3 id="data-flows-and-context" tabindex="-1"><a class="header-anchor" href="#data-flows-and-context" aria-hidden="true">#</a> Data Flows and Context</h3>
<p>If <strong>Data Discovery</strong> was this <code v-pre>what</code>, <strong>Data Flows and Context</strong> establishes the <code v-pre>where</code> of data management.  The movement and usage, including locations such as databases, file sharing apps, endpoints, and personal cloud apps.  Locations are not treated in isolation; this objective considers the flows and usage of sensitive information and answers two critical questions - where is all sensitive data located, and how did it get there?</p>
<p>Data can often pass through a variety of entities, such as being shared by multiple users, stored in cloud applications, modified, and shared again. The ability to track this complex journey is an increasingly essential part of seeing and controlling an organization's true data risk. This may require the organization to develop methods to track how data is shared after it is accessed, including any copies or derivatives.</p>
<table>
<thead>
<tr>
<th>Levels</th>
...
<h2 id="security-patterns" tabindex="-1"><a class="header-anchor" href="#security-patterns" aria-hidden="true">#</a> Security Patterns</h2>
<p>The DSMM looks at data on its own, as an asset, instead of simply as data residing on infrastructure.</p>
<p>To be able to protect data adequately and meet the various recommendations in this model, certain security patterns can help, by leveraging the <a href="https://www.fastly.com/blog/the-dept-of-know-live-sounil-yu-on-why-embracing-the-die-security-model-means-faster-innovation" target="_blank" rel="noopener noreferrer">D.I.E triad<ExternalLinkIcon/></a>.</p>
<h3 id="managed-data-stores" tabindex="-1"><a class="header-anchor" href="#managed-data-stores" aria-hidden="true">#</a> Managed data stores</h3>
<p>Favor managed data stores.</p>
<p>The shared responsibility model allows customers to focus on securing the data itself, without worrying about the infrastructure.</p>
<p>Managed data stores are also, by definition, easy to inventory and backup.</p>
<p>This can help you achieve practices in multiple areas, from <em>Data Location Discovery and Context</em> to <em>Controlling Access</em>, <em>Recover and Improve</em> and more.</p>
<p>This section of the DSMM is not an exhaustive list of security patterns that can help you increase data security maturity but rather an example of practices possible in modern environments where a high level of security can be achieved without requiring large amounts of manual operations and processes.</p>
<p>This is based on the <a href="https://www.youtube.com/watch?v=_omGtDfaAjI" target="_blank" rel="noopener noreferrer">D.I.E. triad<ExternalLinkIcon/></a>.</p>
<h4 id="managed-data-stores-1" tabindex="-1"><a class="header-anchor" href="#managed-data-stores-1" aria-hidden="true">#</a> Managed data stores</h4>
<p>Favor managed data stores, which are much easier to manage via APIs.</p>
<p>Managed data stores have multiple advantages:</p>
<ol>
<li>The shared responsibility model allows the customer to focus on securing the data itself, instead of infrastructure.</li>
<li>Cloud IAM can be used to control access to them, leveraging security processes and technology around cloud identities.</li>
<li>Backups are managed, and recovery is typically relatively easy and fast.</li>
<li>Most types of managed data stores offer interesting redundancy options, locally and globally.</li>
</ol>
<p>This allows you to have data stores that are <strong>distributed</strong>, via redundancy options, with a configuration that is <strong>immutable</strong> or at least very easy to track, and if the data itself is backed up frequently and easy to restore, you could consider the database itself <strong>ephemeral</strong> and even automate its destruction and recovery, to ensure it is always as easy and rapid as required.</p>
<h4 id="cryptography" tabindex="-1"><a class="header-anchor" href="#cryptography" aria-hidden="true">#</a> Cryptography</h4>
<p>In the pet versus cattle equation, the data itself is oven a pet. As we typically do not want to make all data ephemeral, at least, below its usefulness window, it has to be protected with additional layers. For data that is of little value over any period of time or for which confidentiality is not important, these steps are overkill, and you should focus on ensuring data is removed rapidly.</p>
<p>Cryptography in most cloud service providers is available in many forms.</p>
<ol>
<li>Encrypt managed data stores at rest. While this will not improve security much for running databases, it usually means backups and snapshots will be encrypted, and it certainly makes some audit questions easier to answer.</li>
<li>Select specific data that is worth encrypting, anonymizing or hashing at the column level, then, use managed tools by the cloud service provider to do so.</li>
<li>Leverage managed key management services, using customer keys where appropriate. For critical data, consider using cloud hardware security modules.</li>
<li>Ensure only the appropriate services have access to the keys.</li>
</ol>
<p>For example, imagine a scenario where you must store social security numbers (SSNs) to report them to the government once a year, or to identify duplicate customer accounts. You could:</p>
<ol>
<li>Encrypt the SSN, using a combination of symmetric and asymmetric encryption, ensuring that the web service capturing the SSN only has write access and the ability to use a public key.</li>
<li>Create a service that is able to read the SSNs, has no access to the Internet, and is only able to return a <em>match</em> or <em>no-match</em> result when provided a SSN to compare against.</li>
<li>Create a third service that is able to read the SSNs and has no access to the Internet except for the destination they need to be sent for government processing, where they should be sent in an encrypted manner.</li>
</ol>
<p>By leveraging cryptography for what matters the most, you turn pets (valuable, clear-text data) into something closer to cattle (encrypted data, not valuable without the keys to it).</p>
<h4 id="automation" tabindex="-1"><a class="header-anchor" href="#automation" aria-hidden="true">#</a> Automation</h4>
<p>Automation is a key enabler of data security. By cutting out human access, we reduce the chances of mistakes, malicious activities using stolen credentials, malicious insiders, and we increase ephemerality and immutability of systems.</p>
<p>Infrastructure-as-code located in source control can easily be reviewed by multiple team members, providing you with an audit trail of all changes as well as who requested and approved them.</p>
<p>You can achieve many practices of the DSMM at a high level of maturity by:</p>
<ol>
<li>Using CI/CD pipelines to configure managed data stores, from automation systems, with accounts that are not known or accessible to humans.</li>
<li>Using CI/CD pipelines to handle data structure migrations and upgrades.</li>
<li>Requiring the use of CI/CD pipelines to perform manual actions on data, such as using a gitops flow to run queries and manually fix issues during emergencies.</li>
</ol>
<p>Imagine an environment where all of your data stores are configured via <a href="https://www.terraform.io/" target="_blank" rel="noopener noreferrer">Terraform<ExternalLinkIcon/></a>, <a href="https://aws.amazon.com/cloudformation/" target="_blank" rel="noopener noreferrer">CloudFormation<ExternalLinkIcon/></a> or similar tools, where no human has access to their configuration[^1]. The data stores can use a standardized configuration, which will not drift due to manual, untracked troubleshooting.</p>
<p>Access to the data itself should also be governed via a similar process, for example, when data structure needs to be updated.</p>
<p>This results in a system where cloud administrators do not have direct access to the data. By ensuring direct access to the data is not possible except via automation and cutting out a large swatch of attack classes, you also reduce the need for technologies such as DLP, and the need to secure endpoints is reduced as well.</p>
<p>[^1]: Having monitored &quot;break-glass&quot; access accounts with physically secured 2FA methods is a good practice, to avoid being locked out while preventing casual use.</p>
<h4 id="backups" tabindex="-1"><a class="header-anchor" href="#backups" aria-hidden="true">#</a> Backups</h4>
<p>Any data that is considered a pet needs to be easily available and recoverable. Data that is not a pet and can be recreated easily, or is of low value, should not be backed up, as doing so would generate cost, and in some cases increase attack surface for no benefit.</p>
<p>For data that must be conserved over time:</p>
<ol>
<li>Back up using tools native to the managed data store, frequently.</li>
<li>Ensure backups are stored in a destination that can not be deleted, even with an administrative account.</li>
<li>Monitor and alert on backup job completion.</li>
<li>Automate restoration and testing of backups in separate data stores.</li>
</ol>
<p>Imagine the following back up environment.</p>
<p>You have a data store containing critical data about the state of stock in a warehouse. Losing it would completely halt the business for weeks, as it would be impossible to pick and ship orders until it can be manually rebuilt. This data is not particularly sensitive, though a competitor could use it to understand what products are good sellers as well as how much stock is kept of every item, so it is something you'd rather keep private.</p>
<p>This data would be considered a <em>pet</em>, and not cattle.</p>
...
<p>The backups taken in the remote bucket are tested for recovery weekly, using automation that restores it, runs queries on both the live database and the backed up one, and confirms that entries that were created just before the backup was done are present. It also measures how long it took to recovery the database.</p>
<p>As you track that metric, you realize that recovery has become very long, due to the size of the database. Thanks to this automation, you decide to replicate the database to other geographic regions, increasing cost, but reducing the odds of needing to fully recovery it. You take regular snapshots in the remote region, to be able to go back to a snapshot easily if a software bug corrupt the data.</p>
<p>A ransomware threat group takes control of your cloud environment, steals your root account, and deletes databases. Unfortunately for them, they can't do anything to destroy the backups, and so you are back online rapidly after recovering the database in a clean environment.</p>
<h4 id="infrastructure" tabindex="-1"><a class="header-anchor" href="#infrastructure" aria-hidden="true">#</a> Infrastructure</h4>
<p>By using microservices, access control to datastores can be made granular to every service.</p>
<ol>
<li>Use microservices in the environment that is the most managed you can technically use. Favor cloud functions over containers, containers over temporary VMs, and temporary VMs over long-lived server VMs.</li>
<li>Leverage service mesh features of managed environments to enforce policy that protects data in transit, such as by enforcing mutual TLS authentication and encryption between all services and data stores.</li>
<li>Instead of using traditional DLP tools to intercept data being exfiltrated, monitor cloud logs for exfiltration and use the service mesh to control egress to the Internet. By only allowing the data to be exfiltrated within the cloud environment, such activity will be much easier to detect.</li>
</ol>
<h4 id="temporary-and-limited-access" tabindex="-1"><a class="header-anchor" href="#temporary-and-limited-access" aria-hidden="true">#</a> Temporary and Limited Access</h4>
<p>Support staff often require read access to data, including customer data.</p>
<ol>
<li>Ensure data is only accessible via applications meant to do so, using a combination of access controls and encryption.</li>
<li>Favor models where the owner of the data approves its use for support purposes.</li>
<li>Allow direct access to data for troubleshooting purposes only via fully monitored sessions, on systems without full Internet access.</li>
</ol>
<p>When you call your bank, they're usually able to pull up your records instantly. Why?</p>
<p>In a world where data security is a priority, such types of access should be temporary and require your approval. A bank support employee could instead request access, which you'd approve via a push notification to their application. The employee would receive a token allowing them to view your data for 60 minutes. Such a measure would go a long way towards preventing massive theft of data by malicious insiders or stolen accounts of employees, while also providing a provable audit trail of who read what customer information.</p>
</div></template>
